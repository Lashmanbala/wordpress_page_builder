...
def read_document(doc_service, doc_id, country_name, category_name):
    ...
    try:
        ...
        logger.exception(f"❌ Failed to read or validate document {doc_id}: {e}")
        exit(1)
...
def update_wp_page(page_id, html_content, base_url, auth, featured_img):
    """Update WordPress page content."""
    try:
        res = update_new_content(html_content, base_url, page_id, auth.username, auth.password, featured_img)
        if res.status_code == 200:
            logger.info(f"✅ Updated WP page ID {page_id}")
            return True
        else:
    ...
    for tab in tabs:
        try:
            ...
            for city_name, html_content in html_content_dict.items():
                ...
                success = update_wp_page(page_id, html_content, env["WP_BASE"], auth, env["new_img"])
                if success:
